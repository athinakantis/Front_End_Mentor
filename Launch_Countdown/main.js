const now = new Date();
const countdownDate = new Date(`November 03, ${now.getFullYear()} 00:00:00`);
const randomPos = (min, max) => Math.random() * (max - min) + min;

// Returns if current date is same as birthday-date
function isBirthday() {
  return (
    countdownDate.toJSON().slice(5, 10) === new Date().toJSON().slice(5, 10)
  );
}
...
const daysPrevCount = document.querySelectorAll('.daysPrevCount');
const daysNewCount = document.querySelectorAll('.daysNewCount');

const hoursUpper = document.querySelector('#hoursFrontUpper');
const hoursLower = document.querySelector('#hoursFrontLower');
const hoursPrevCount = document.querySelectorAll('.hoursPrevCount');
const hoursNewCount = document.querySelectorAll('.hoursNewCount');

const minutesUpper = document.querySelector('#minutesUpper');
const minutesLower = document.querySelector('#minutesLower');
const minutesPrevCount = document.querySelectorAll('.minutesPrevCount');
const minutesNewCount = document.querySelectorAll('.minutesNewCount');

const secondsUpper = document.querySelector('#secondsUpper');
const secondsLower = document.querySelector('#secondsLower');
const secondsPrevCount = document.querySelectorAll('.secondsPrevCount');
const secondsNewCount = document.querySelectorAll('.secondsNewCount');
...
function updateTime() {
  ...
  if (secondsUntil < secondsCount || secondsCount === 0) {
    ...
    setTimeout(
      secondsNewCount.forEach((p) => (p.textContent = secondsUntil)),
      500
    );
    setTimeout(
...
function happyBirthday() {
  ...
  setInterval(() => {
    for (let i = 0; i < 10; i++) {
      ...
      firework.animate(
        {
          transform: `translate(${randomPos(-100, 100)}px, -${randomPos(
            200,
      ...
      if (
        firework.style.left > window.innerWidth ||
        firework.style.left < window.innerWidth
      ) {
        firework.remove();
      }
